...
/**
 * API origin (no trailing slash), e.g. https://devapi.example.com
 * Order: `expo.extra` (from app.config.js + .env) → EXPO_PUBLIC_* → localhost fallback.
 */
function readApiOrigin(): string {
  ...
  const fromPublic = process.env.EXPO_PUBLIC_API_ORIGIN;
