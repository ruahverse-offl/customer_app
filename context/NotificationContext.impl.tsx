...
const NOTIFICATIONS_PROMPTED_KEY = 'nb_notifications_prompted_once';
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadState = useCallback(async () => {
    setLoading(true);
    try {
      const [storedPref, storedPrompted, permissions] = await Promise.all([
      ...
      // On first login (never prompted), ask the OS for permission once automatically.
      if (!storedPrompted && authToken) {
  ...
  useEffect(() => {
    receivedSubscriptionRef.current = Notifications.addNotificationReceivedListener(() => {
      // App-level listener attached for future in-app banner/state updates.
    });
    responseSubscriptionRef.current = Notifications.addNotificationResponseReceivedListener(() => {
      // Keep subscription active for navigation-based deep links in future.
    });
  ...
  const setEnabled = useCallback(
    async (next: boolean) => {
      if (!next) {
        ...
        // Tell the backend this device no longer wants pushes.
        if (authToken) {
          try {
  ...
  const value = useMemo(
    ...
    [loading, enabled, permissionGranted, expoPushToken, setEnabled, refreshToken, sendLocalTest],
  );
