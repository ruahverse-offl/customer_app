...
/**
 * Fetches role and module menu from the backend (same contract as the web app).
 * @param token — Bearer token; pass explicitly so permissions work before secure store write completes.
 */
...
/**
 * Verifies the user's password (same as login) without updating stored auth.
 * Use before sensitive account updates when the API does not accept a password field.
 */
